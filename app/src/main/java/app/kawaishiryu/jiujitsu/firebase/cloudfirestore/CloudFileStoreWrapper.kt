        ...
        return suspendCoroutine { continuation ->
            FirebaseFirestore.getInstance().collection(collectionPath).document(documentPath).get()
                .addOnSuccessListener { value ->
                    if (value.exists()) {
                        //Se encontro el valor en la lectura
                        //Nose si esta bien
                        ...
                        //No se encontro el valor en la lectura
                    }

                }.addOnFailureListener {
                //error base de datos no encontro
            }
        }
    }
...
val docRef = db.collection("tu_coleccion").document("tu_documento")
    ...
    suspend fun modifiedCurrentUser(
        ...
        return suspendCoroutine { continuation ->
            Log.i("mapeo","$map")
            Firebase.firestore.collection(collectionPath).document(documentPath).set(
                map,
                SetOptions.merge()
            ).addOnSuccessListener {
    ...
    fun getUUID(): String = firebaseAuth.currentUser!!.uid
