...
class ProfileUserModifiedFragment : Fragment(R.layout.fragment_profile_user_modified) {
    ...
    private val currentUserModified = UserModel()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.btnChangeDate.setOnClickListener {
            Log.i("ID","${argsCurrentUser.currentUser.id.isNullOrEmpty()}")
            cambiarDatos()
        }
    }

    private fun cambiarDatos() {
        Log.i("datos ID Actuales","${argsCurrentUser.currentUser.id}")
        Log.i("datos Password","${argsCurrentUser.currentUser.password}")
        //boton cuando se realice un evento

        currentUserModified.currentUser.password = "Nacho Puto"
        Log.i("Ya inicio","Estamos aqui")
        Log.i("datos de current User","${currentUserModified.currentUser.password}")
        viewModel.modifiedCurrentUser(user = currentUserModified,argsCurrentUser.currentUser.id)
